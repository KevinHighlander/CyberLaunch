...
def test_default_lookback_is_seventy_two_hours() -> None:
    assert DEFAULT_INCIDENT_LOOKBACK == timedelta(
        hours=72
    )


def test_related_report_inside_lookback_joins_existing_incident(
    tmp_path: Path,
) -> None:
    database_path = (
        tmp_path
...
def test_related_report_outside_lookback_creates_new_incident(
    tmp_path: Path,
) -> None:
    database_path = (
        tmp_path
...
def test_custom_lookback_can_extend_incident_candidate_window(
    tmp_path: Path,
) -> None:
    database_path = (
        tmp_path
